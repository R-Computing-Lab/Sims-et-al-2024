...
df_data_SEX_DEP792 <- df_data_gen1%>%
  select(CESD7_1992, SEX)

ggplot(df_data_SEX_DEP792, aes(x = SEX)) + 
  geom_density(fill = "cornflowerblue", 
              color = "black") +
  labs(x = "SEX", 
       y = "CESD 7 1992", 
       title = "Distribution of CESD 7 1992 by Sex",
        subtitle = "0=female, 1=male") +
  theme_minimal()
...
ggplot(df_data_gen1, aes(x =AGE_1979, y= DELIN_AVERAGE)) + 
  ...
  labs(x = "Age", 
       y = "DELIN_AVERAGE", 
       title = "Correlation of Age and Delinquency") +
     theme_minimal()

ggplot(df_data_gen1, aes(x =AGE_1979, y= CESD7_1994)) + 
    geom_point()+
  geom_smooth(method=lm, se=FALSE)
  labs(x = "Age", 
       y = "CESD7_1994", 
       title = "Correlation of Age and CESD7_1994") +
     theme_minimal()

  ggplot(df_data_gen1, aes(x =AGE_1979, y= CESD7_1992)) + 
    geom_point()+
  geom_smooth(method=lm, se=FALSE)
  labs(x = "Age", 
       y = "CESD7_1992", 
       title = "Correlation of Age and CESD7_1992") +
     theme_minimal()

    ggplot(df_data_gen1, aes(x =AGE_1979, y= CESD20_1992)) + 
    geom_point()+
  geom_smooth(method=lm, se=FALSE)
  labs(x = "Age", 
       y = "CESD20_1992", 
       title = "Correlation of Age and CESD20_1992") +
     theme_minimal()
...
lm_og_ydelin_xCESD7_1992<-lm_model <- lm(DELIN_AVERAGE ~
                     CESD7_1992 +
                     as.factor(SEX) +
                     AGE_1979+
                     as.factor(MINORITY) ,
...
lm_og_ydelin_xCESD20_1992 <-lm_model <- lm(DELIN_AVERAGE ~
                      CESD20_1992 +
                      as.factor(SEX) +
                        AGE_1979+
                      as.factor(MINORITY),
...
lm_og_ydelin_xCESD7_1994 <-lm_model <- lm(DELIN_AVERAGE ~
                     CESD7_1994 +
                     as.factor(SEX) +
                       AGE_1979+
                     as.factor(MINORITY) ,
...
lm_og_xdelin_yCESD7_1992 <-lm_model <- lm(CESD7_1992 ~
                     DELIN_AVERAGE +
                     as.factor(SEX) +
                       AGE_1979+
                     as.factor(MINORITY) ,
...
lm_og_xdelin_yCESD20_1992 <-lm_model <- lm(CESD20_1992 ~
                     DELIN_AVERAGE +
                      as.factor(SEX) +
                       AGE_1979+
                      as.factor(MINORITY),
...
lm_og_xdelin_yCESD7_1994 <-lm_model <- lm(CESD7_1994 ~
                      DELIN_AVERAGE+
                     as.factor(SEX) +
                       AGE_1979+
                     as.factor(MINORITY) ,
...
#NEWTABLE
lm_btw_xdelin_sex3_yCESD7_1992 <- lm(CESD7_1992_mean ~ 
                             DELIN_AVERAGE_mean +
                            SEX_all3 +
                           #  factor(SEX_1)+factor(SEX_2)+
                             AGE_1979_mean+
...
lm_btw_xdelin_sex3_yCESD7_1994 <- lm(CESD7_1994_mean ~ 
                            DELIN_AVERAGE_mean+
                            SEX_all3 +
                            AGE_1979_mean+
                           #  factor(SEX_1)+factor(SEX_2)+
...
lm_og_xdelin_yCESD7_1992  %>%  
prettify_regression_results(standardize=TRUE)
lm_og_xdelin_yCESD20_1992  %>%  
prettify_regression_results(standardize=TRUE)

lm_og_xdelin_yCESD7_1994  %>%  
prettify_regression_results(standardize=TRUE)
...
lm_btw_xdelin_sex3_yCESD7_1994 %>%  
prettify_regression_results(standardize=TRUE)

lm_btw_xdelin_sex3_yCESD7_1992 %>%  
prettify_regression_results(standardize=TRUE)
```
...
This study investigates the relationship between depression and delinquent behavior using a sibling comparison framework. We examined these relationships in three stages: a classical regression model, a between-family model, and a discordant kinship model. We also explored the influence of gender, race, and age on these associations.

Our primary focus was on the CES-D 20 as the depression measure due to its comprehensive 20-item scale, compared to the alternative short-form versions. For readers interested in replicated analyses using the other two measures, please refer to the appendix. Any discrepancies between the results are addressed in the main text.

## Unadjusted Multiple Regressions
...
We first analyzed the data using unadjusted bivariate descriptive statistics without accounting for family structure. The results aligned with previous research (see Table 2), showing a weak but positive correlation (r = 0.07) between delinquency average and depression, as measured by CES-D 20.

Next, we employed a classical regression model to examine the associations without adjusting for family structure. The dependent variable was the delinquency average (DELIN_AVERAGE), and the independent variables were CES-D 20 scores in 1992 (CESD20_1992), sex (as.factor(SEX)), age in 1979 (AGE_1979), and minority status (as.factor(MINORITY)). The analysis included `r result_n` observations, with 4188 observations deleted due to missing data. The F-statistic (`r result_f`,df1=`r result_ndf`,df2=`r result_ddf`) was significant, and the adjusted R$^{2}$ was `r result_r2`. 


The regression model revealed that depression significantly predicted criminal behavior, even after controlling for race, gender, and age. As anticipated, gender composition, age, and minority status also significantly predicted delinquency levels. Specifically, being male was associated with a higher average delinquency score (β = `r result_bg`), whereas older age predicted lower delinquency levels (β = `r result_age_mean`). In contrast, belonging to a minority group was linked to a `r result_minority` standard deviation decrease in the average delinquency measure. Collectively, these factors—sex, age, minority status, and depression—accounted for `r result_r2 * 100`% of the variability in criminal behavior, as indicated by an R-squared value of 0.0867 and an adjusted R-squared value of `r result_r2`.

The multiple regression model results indicated that the greater the depression experienced by an adolescent, the higher the likelihood of criminal activity. In the past, being in a minority group was associated with increased crime rates; however, our findings suggest that belonging to a minority group does not contribute to higher crime rates. The interpretation of these results highlights the direct influence of gender and increased depression on criminal activity.
